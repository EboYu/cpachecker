...
void UE__emm_as_recv(){//nas_message_decode
    nas_msg = uePullPlainNASMsgFromCache();
//    decoder_rc = 0;
}

void UE__emm_as_establish_cnf(){//nas_message_decode
    nas_msg = uePullPlainNASMsgFromCache();
//    decoder_rc = 0;
}

void UE__emm_as_data_ind(){//nas_message_decrypt, pull header
    header = uePullPlainNASHeaderFromCache();
//    bytes = 0;
}
//insert after if (as_msg.msgID > 0) {
void UE__emm_as_send(){
    uePushPlainASMsgIntoCache(as_msg);
}


//before nas_message_encode
void UE__emm_as_encode(){
...
void MME__emm_as_recv(){//nas_message_decode
    nas_msg = cnPullPlainNASMsgFromCache();
//    decoder_rc = 0;
}
void MME__emm_as_establish_req(){
    nas_msg = cnPullPlainNASMsgFromCache();
//    decoder_rc = 0;
}

void MME__emm_as_data_ind(){//nas_message_decrypt, pull header
    header = cnPullPlainNASHeaderFromCache();
//    bytes = 0;
}
//insert after if (as_msg.msgID > 0) {
void MME__emm_as_send(){
    cnPushPlainASMsgIntoCache(as_msg);
}


//before nas_message_encode
void MME__emm_as_encode(){
